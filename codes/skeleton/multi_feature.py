##ft3dFunction -> heatman_function.py
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride=1):
        ...
        self.residual_function = nn.Sequential(
            nn.Conv3d(in_channels, out_channels, (3,1,1), stride=(2,1,1), padding=(1, 0, 0), bias=False),
            
            nn.BatchNorm3d(out_channels),
            nn.ReLU(),
            nn.Conv3d(out_channels, out_channels * BasicBlock.expansion, kernel_size=1, stride=1,bias=False),
            nn.BatchNorm3d(out_channels * BasicBlock.expansion),
        )

        # identity mapping, input과 output의 feature map size, filter 수가 동일한 경우 사용.
        self.shortcut = nn.Sequential(
            nn.Conv3d(out_channels, out_channels, kernel_size=(1,1,1), stride=(2,1,1)),
            nn.BatchNorm3d(out_channels))
        ...
        if stride != 1 or in_channels != BasicBlock.expansion * out_channels:
            self.shortcut = nn.Sequential(
                nn.Conv3d(in_channels, out_channels * BasicBlock.expansion, kernel_size=(1,1,1), stride=(2,1,1), bias=False),
                nn.BatchNorm3d(out_channels * BasicBlock.expansion)
            )
...
class skeleton_feature(nn.Module):
    def __init__(self, in_channels, kernel_tup=()):
        super().__init__()
...
class Feature3D_bf(nn.Module):
    ...
    def forward(self, x):
        x = self.conv3dResi(x.permute(0, 2, 1, 3, 4))
        # print("heatmap ",x.shape)
        x = x.squeeze(-1)
...
class NetworkWrapper(nn.Module):
    def __init__(self, 
        ...
        self.heat_feature = Feature3D_bf(keypoint, 30, kernel_tup=(1, 4, 4))
        self.skeleton_feature = skeleton_feature(128)
    ...
    def forward(self, x, y):
        concated_modal = self.heat_feature(x) + self.skeleton_feature(y)
        print(concated_modal.shape)
        out = self.vit(concated_modal)
        return out

class heatman_Data(Dataset):
    def __init__(self, heat_dir = '/dataset/KETI/subset10_kp53', 
        ...
        # self.heat_npy.sort() # sort하면 imu와 충돌 납니다...
        self.mode = mode
    ...
    def load_batch(self, b_index=0, b_size=30, transform=None, target_transform=None):
        ...
        for a in self.heat_npy[b_index:]:
            ...
            if cnt > b_size:
                break
            print('\r Load DATA {}:{} / {}:{}  '.format(cnt, b_index, b_size, len(self.heat_npy)), end='')
